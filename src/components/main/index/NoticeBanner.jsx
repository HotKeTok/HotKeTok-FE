...
import { NOTICE_CARD_MOCK } from "../../../constants/main"
...
export default function NoticeBanner({}) {
    ...
    return (
        <Container $gap={10}>
             <Row $align={"center"} $justify={"space-between"} style={{cursor: 'pointer'}} onClick={handleBannerClick}>
                <H3>공지사항</H3>
                <ArrowRightStyled style={{width: 6, height: 7}}/>
            </Row>  
            {NOTICE_CARD_MOCK.map((item, index) => (
                <Row $justify={"space-between"} style={{ borderBottom: item.border ? '1px solid #efefef' : 'none', paddingBottom: item.border ? 12 : 0}} key={index}>
